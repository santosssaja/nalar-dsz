...
- **Seed Kurasi Rekonsiliasi Setiap Boot & Perbaikan Tabrakan FK DB (`2026-09-19`):** 
  ...
  - *Pencegahan di kode:* `seedCuratedContent` di `src/server/db/seed.ts` kini menggunakan `onConflictDoUpdate` untuk konsep dan memperbarui `conceptId` pada `learning_steps`, serta `learning-service.ts` menjamin rantai relasi `domain` → `module` → `concept` ter-upsert sebelum memasukkan `learning_evidence`.
- **Verifikasi Kualitas:** 28 test suite (109 tests) lulus 100%, TypeScript typecheck lulus 100% (`tsc --noEmit`), lint zero error, production build lulus (`next build`), zero-emoji UI design dengan Lucide icons.
